...
- (bool)processOp:(afc_operation)op from:(const char *)func
{
	...
	if ([self checkStatus:AFCConnectionProcessOperation(_afc, op, AFCConnectionGetIOTimeout(_afc)) from:"AFCConnectionProcessOperation"]) {
		...
		if (e == 0) {
			...
			if (err) {
				id msg = [(__bridge id)err objectForKey:@"NSLocalizedDescription"];
				if (msg) {
...
- (BOOL)setmodtime:(NSString*)path to:(NSDate*)timestamp
{
	...
	void *op = AFCOperationCreateSetModTime(NULL,(__bridge CFStringRef)path, tm, (__bridge void *)(self));
	if (op) {
...
#if 0
Yeah, Iâ€™ve just found out how to handle this and terminate sync when user slides cancel switch.
...
First, we need AMDObserveNotification to subscribe notifications about â€œcom.apple.itunes-client.syncCancelRequestâ€. Then we should start listening for notifications (second function) until we get â€œAMDNotificationFaceplantâ€.
Thatâ€™s it. When notification got, you should unlock and close lock file handle (donâ€™t sure if you need to post â€œsyncDidFinishâ€ to proxy, seems it doesnâ€™t matter) and terminate sync gracefully.
...
After which it will accept commands (in the form [â€œDLMessageProcessMessageâ€, {â€œMessageTypeâ€ â‡’ commandname}]).
...
{
	am_service result;
	mach_error_t ret = AMDeviceStartService(_device,(__bridge CFStringRef)name, &result);
...
- (id)initWithDevice:(am_device)device orBust:(NSString**)msg
{
	if (self=[super init]) {
        ...
        //
		_deviceName = [self deviceValueForKey:@"DeviceName"];
        _udid = [self deviceValueForKey:@"UniqueDeviceID"];
...
- (CFDictionaryRef)defaultReturnAttributes
{
	...
	return (__bridge CFDictionaryRef)options;
}
